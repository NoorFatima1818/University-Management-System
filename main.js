...
async function addCourseToDepartment() {
    ...
    const department = universitySystem.departments.find((dep) => dep.name === departmentName);
    if (!department) {
        console.log(chalk.redBright(`Department ${departmentName} not found!`));
        return;
    }
    department.courses.push(courseName);
    console.log(chalk.magentaBright(`Course ${courseName} added to department ${departmentName} successfully!`));
}
...
async function assignInstructorToCourse() {
    ...
    const instructor = universitySystem.instructors.find((inst) => inst.name === instructorName);
    if (!instructor) {
        console.log(chalk.redBright(`Instructor ${instructorName} not found!`));
        return;
    }
    instructor.courses.push(courseName);
    console.log(chalk.greenBright(`Instructor ${instructorName} assigned to course ${courseName} successfully!`));
}
// register student for course
async function registerStudentForCourse() {
    ...
    const student = universitySystem.students.find((stu) => stu.name === studentName);
    if (!student) {
        console.log(chalk.redBright(`Student ${studentName} not found!`));
        return;
    }
    student.courses.push(courseName);
    console.log(chalk.cyanBright(`Student ${studentName} registered for course ${courseName} successfully!`));
}
// view university data
